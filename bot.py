...
async def on_startup(dp):
    ...
    await bot.set_webhook(config.ask_bot.WEBHOOK_URL)
...
if __name__ == '__main__':
    start_webhook(dispatcher=dp, webhook_path=config.ask_bot.WEBHOOK_PATH,
                  on_startup=on_startup, on_shutdown=on_shutdown,
                  host=config.ask_bot.WEBAPP_HOST, port=config.ask_bot.WEBAPP_PORT, skip_updates=True)
    asyncio.run(main())
